        ...
        for r in range(len(board)):
            for c in range(len(board[r])):
                if board[r][c] != ".":
                    ...
                    if box not in boxes: # Store number in correct box
                        boxes[box] = [board[r][c]]
                    else:
                        boxes[box].append(board[r][c])

                    if r not in rows: # Store number in correct row
                        rows[r] = [board[r][c]]
                    else:
                        rows[r].append(board[r][c])
                    
                    if c not in cols: # Store number in correct column
                        cols[c] = [board[r][c]]
                    else:
                        cols[c].append(board[r][c])

        if self.validity(rows) and self.validity(cols) and self.validity(boxes):
            return True
        else:
            return False

    def validity(self, dictionary) -> bool:
        duplicates = set()

        for value in dictionary.values():
            duplicates.clear()
            for integer in value:
                if integer not in duplicates:
